...
def main():
    ...
    st.set_page_config(page_title='Customer Spend Prediction', page_icon='🖖', layout="wide")
    st.markdown('<style> .stMarkdown, .css-18e3th9, .css-1inwz65 .stApp, .css-fg4pbf, .streamlit-wide,.eczokvf0 { padding-top: 0.5em; padding-left: 2em; padding-right: 2em; font-family: Inter} h1, h2, h3, h6, .st-ae {font-family: Inter} .stSlider, .stDownloadButton {padding-left: 2em; font-family: Inter} .css-13sdm1b, .e16nr0p33 {font-family: Inter; padding-top: 0px} .css-1ggh3qq {gap: 0em} </style>', unsafe_allow_html=True)
    # cola, colb = st.columns(2)
    st.title('Customer Spend Prediction')
    st.markdown('###### Explore Customers by Predicted Spend versus Actual Spend during the 90-day evaluation period.')
    ...
    with col1:
        ...
        st.markdown('###### Segment Customers that were predicted to spend but didn\'t. Target these customers with appropriate marketing campaigns.')
        ...
        df_data = convert_df(prediction_data[(prediction_data['Actual vs Predicted Amount Difference'] > temp[0]) & (prediction_data['Actual vs Predicted Amount Difference'] <= temp[1])])

        st.download_button(label = 'Download Segmentation', data = df_data, file_name = 'segmentation.csv', mime = 'text/csv')

        

    with col2:
        fig = px.scatter(prediction_data[(prediction_data['Actual vs Predicted Amount Difference'] > temp[0]) & (prediction_data['Actual vs Predicted Amount Difference'] <= temp[1])], 
